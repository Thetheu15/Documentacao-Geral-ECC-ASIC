...
\begin{titlepage}
    \begin{center}
        ...
        {\large \bfseries DEPARTAMENTO DE ENGENHARIA DE TELEINFORMÁTICA} \\[0.5cm]
        ...
        {\large Matheus Parente Reis, Antônio Matheus} \\
        \vfill
...
Seguindo a ordem de montagem implementada no \texttt{MRSC\_encoder} 
(transposição dos dados, seguida de $Di$, $P$ e $Cb$ em ordem intercalada 
$[Di_1,Di_4,Di_2,Di_3]$ e $[P_1,P_4,P_2,P_3]$), o \textit{codeword} de 32 
bits resultante é:
\begin{equation*}
    \underbrace{0010\;0110\;0101\;0111}_{\text{dados (transpostos)}}\;
    \underbrace{0110}_{Di}\;\underbrace{1100}_{P}\;
    \underbrace{0010\;1011}_{Cb}
\end{equation*}
...
Para manter a continuidade com o exemplo apresentado para o MRSC, reutilizamos 
o mesmo dado de 16 bits: o número 2003, cuja matriz de dados é:
\begin{align*}
    A &= [A_1, A_2, A_3, A_4] = [0, 0, 0, 0] \\
    B &= [B_1, B_2, B_3, B_4] = [0, 1, 1, 1] \\
    C &= [C_1, C_2, C_3, C_4] = [1, 1, 0, 1] \\
...
Os bits $Di$ e $Cb$ seguem exatamente as mesmas equações do MRSC (não são 
alterados pelo TBEC-RSC), portanto seus valores são idênticos aos já 
calculados na Seção~3.2.2:
\begin{align*}
    Di &= [Di_1, Di_2, Di_3, Di_4] = [0, 1, 0, 1] \\
    Cb &= [CbA, CbB, CbC, CbD] = [00,\; 10,\; 10,\; 11]
\end{align*}

Já os bits de paridade $P$ utilizam as novas equações cruzadas (3) e (4), 
que combinam pares de grupos ($A,B$) e ($C,D$) ao invés da paridade vertical 
por coluna:
\begin{align*}
    P_0 &= A_1 \oplus A_2 \oplus B_1 \oplus B_2 = 0 \oplus 0 \oplus 0 \oplus 1 = 1 \\
    P_1 &= C_1 \oplus C_2 \oplus D_1 \oplus D_2 = 1 \oplus 1 \oplus 0 \oplus 0 = 0 \\
    P_2 &= A_3 \oplus A_4 \oplus B_3 \oplus B_4 = 0 \oplus 0 \oplus 1 \oplus 1 = 0 \\
    P_3 &= C_3 \oplus C_4 \oplus D_3 \oplus D_4 = 0 \oplus 1 \oplus 1 \oplus 1 = 1
\end{align*}
\textit{Vetor $P$ gerado: \texttt{1001}}

Seguindo a ordem de montagem implementada no \texttt{TBEC\_RSC\_encoder} 
(transposição dos dados, seguida de $Di$, $P$ e $Cb$ em ordem intercalada 
$[Di_1,Di_4,Di_2,Di_3]$ e $[P_0,P_3,P_1,P_2]$), o \textit{codeword} de 32 
bits resultante é:
\begin{equation*}
    \underbrace{0010\;0110\;0101\;0111}_{\text{dados (transpostos)}}\;
    \underbrace{0110}_{Di}\;\underbrace{1100}_{P}\;
...
\paragraph{Passo 2: Injeção de Falha (Burst Não-Adjacente)}

Suponha um evento de radiação que provoque um \textit{burst error} de 
comprimento $l=3$ afetando os bits $B_1$, $C_1$ e $D_1$ -- exatamente o 
padrão \texttt{111} discutido na Seção~\ref{sec:eccs}, que a paridade 
vertical do MRSC tem dificuldade de isolar corretamente por afetar três 
grupos distintos na mesma coluna da matriz. Os bits corrompidos são:
\begin{align*}
    B'_1 &= 0 \rightarrow 1 \qquad C'_1 = 1 \rightarrow 0 \qquad D'_1 = 0 \rightarrow 1
\end{align*}

A matriz lida pelo decodificador passa a conter:
\begin{align*}
    A' &= [0, 0, 0, 0] \\
    B' &= [\mathbf{1}, 1, 1, 1] \\
    C' &= [\mathbf{0}, 1, 0, 1] \\
...
O decodificador recalcula as três síndromes ($SCb$, $SP$, $SDi$) a partir 
dos dados corrompidos, comparando-as via XOR com a redundância armazenada:

\begin{itemize}
    \item \textbf{Síndrome de checagem ($SCb$):} apenas o grupo $A$ 
    permanece intacto ($SCbA = 00$); os grupos $B$, $C$ e $D$ acusam 
    divergência na posição 1--3:
    \begin{equation*}
        SCbA = 00 \qquad SCbB = \mathbf{10} \qquad SCbC = \mathbf{10} \qquad SCbD = \mathbf{10}
    \end{equation*}
    A soma total de bits ativos em $SCb$ é $\sum SCb = 3$.
    
    \item \textbf{Síndrome de paridade ($SP$):} recalculando $P_0$ sobre os 
    dados corrompidos, $A'_1 \oplus A'_2 \oplus B'_1 \oplus B'_2 = 0 \oplus 
    0 \oplus 1 \oplus 1 = 0$; comparando com o $P_0$ armazenado ($1$): 
    $SP_0 = 0 \oplus 1 = \mathbf{1}$. Os demais bits de $SP$ resultam em 
    zero, pois as combinações restantes preservam a paridade:
    \begin{equation*}
        SP = \mathbf{1000}
    \end{equation*}
    
    \item \textbf{Síndrome diagonal ($SDi$):} de forma análoga, apenas o 
    primeiro bit diverge:
    \begin{equation*}
        SDi = \mathbf{1000}
    \end{equation*}
\end{itemize}

\textbf{Verificação da condição especial:} antes de prosseguir, o decoder 
avalia se o erro está confinado à redundância (o que abortaria a correção). 
A condição exige $SDi$ totalmente nulo, $SP$ com pelo menos um bit ativo e 
$SCb$ com dois ou mais bits ativos. Como $SDi = 1000 \neq 0000$, essa 
condição é \textbf{falsa} -- o algoritmo corretamente identifica que o erro 
está nos dados, e não na redundância, prosseguindo para a correção normal.

\textbf{Verificação da condição de erro:} $SP \neq 0000$ e $SDi \neq 0000$ 
simultaneamente, satisfazendo a primeira condição de erro. Adicionalmente, 
$\sum SCb = 3 > 1$, satisfazendo também a segunda. A correção é autorizada.

\textbf{Seleção de região:}
\begin{align*}
    quad_1 &= SP_0+SP_1+SDi_0+SDi_1 = 1+0+1+0=2 \\
    quad_2 &= SP_2+SP_3+SDi_2+SDi_3 = 0+0+0+0=0
\end{align*}
Como $quad_1 > quad_2$, a \textbf{Região 1} é selecionada (colunas 1 e 2 
da matriz).

\textbf{Correção:} o vetor $SCb$ é aplicado via XOR sobre as colunas 1--2 
de cada grupo cuja síndrome não é nula:
\begin{itemize}
    \item Grupo $A$: $SCbA = 00$, nenhuma correção necessária.
    \item Grupo $B$: $[B'_1, B'_2] \oplus [1,0] = [1,1] \oplus [1,0] = [0,1]$ 
    -- $B_1$ restaurado para $0$.
    \item Grupo $C$: $[C'_1, C'_2] \oplus [1,0] = [0,1] \oplus [1,0] = [1,1]$ 
    -- $C_1$ restaurado para $1$.
    \item Grupo $D$: $[D'_1, D'_2] \oplus [1,0] = [1,0] \oplus [1,0] = [0,0]$ 
    -- $D_1$ restaurado para $0$.
\end{itemize}

Os três bits corrompidos pelo \textit{burst} não-adjacente são recuperados 
simultaneamente, restaurando exatamente a palavra original 
\texttt{0000\,0111\,1101\,0011} (2003). Esse resultado ilustra na prática o 
ganho do TBEC-RSC sobre o MRSC: a nova equação de paridade cruzada evita 
que o cancelamento algébrico típico de erros em padrão \texttt{101}/\texttt{111} 
mascare a falha, permitindo a correção de um burst triplo que o esquema de 
paridade vertical original teria dificuldade em isolar corretamente.

\subsubsection{Análise de Vantagens e Desvantagens}

\textbf{Vantagens:} A principal contribuição do TBEC-RSC é resolver a maior 
limitação prática do MRSC sem custo adicional: por adotar um formato de 
\textit{codeword} 1-D, o algoritmo elimina a necessidade de fatiar a palavra 
em múltiplos endereços de memória, permitindo que dado e redundância sejam 
gravados e lidos em um único ciclo de acesso. Do ponto de vista de cobertura 
de erros, o TBEC-RSC corrige 100\% dos \textit{burst errors} triplos (contra 
aproximadamente 70\% do MRSC original) e mantém a maior taxa de correção 
entre os códigos comparados para bursts de 5 a 8 bits, superando inclusive 
códigos com tabelas de correspondência (\textit{matchup tables}) mais 
complexas, como FUEC-DAEC, FUEC-TAEC e FUEC-QUAEC. Além disso, essa evolução 
foi obtida sem aumento de custo de síntese: por herdar a mesma lógica 
puramente combinacional e baseada em XOR do MRSC, o TBEC-RSC apresenta área, 
potência e atraso praticamente idênticos ao seu antecessor, resultando na 
melhor relação entre cobertura de erro e custo de síntese (métrica CSC) de 
todos os códigos avaliados. O algoritmo também é escalável: ao aumentar a 
palavra de dados para 32 ou 64 bits, a taxa de correção geral cresce (pois 
as regiões da matriz se tornam maiores, concentrando mais facilmente os 
erros em uma única região), sem alterar a estrutura básica do algoritmo.

\textbf{Desvantagens:} O TBEC-RSC compartilha com o MRSC uma taxa de 
redundância elevada -- 16 bits de redundância para 16 bits de dados 
($RI=1{,}0$), o dobro proporcionalmente à de códigos lineares mais compactos 
como FUEC-DAEC ($RI=0{,}4375$). Isso penaliza o TBEC-RSC em métricas que 
consideram o custo de armazenamento da redundância na memória (CSCR), mesmo 
quando seu custo de síntese do decodificador é baixo. Em termos de 
confiabilidade ao longo do tempo (MTTF), o TBEC-RSC apresenta o segundo 
melhor resultado entre os códigos avaliados, ficando cerca de 35\% abaixo do 
FUEC-QUAEC -- que, apesar de exigir mais lógica de decodificação, utiliza 
menos bits de redundância. Por fim, embora o TBEC-RSC supere amplamente os 
demais códigos para a maioria dos cenários de \textit{burst}, ele é 
ligeiramente superado pelo FUEC-QUAEC especificamente no cenário de erros de 
4 bits (aproximadamente 15\% menor cobertura), e sua taxa de correção, assim 
como a de todos os códigos analisados, decresce à medida que o comprimento 
do \textit{burst} aumenta -- caindo para cerca de 40\% de cobertura em 
erros de 8 bits na versão de 16 bits do algoritmo.
...
\begin{itemize}
    ...
    da formulação matricial 2-D original do MRSC (Seção~\ref{sec:eccs}), que 
    exigiria 4 endereços separados caso implementada literalmente. Os 16 bits finais concatenam os bits $Di$, $P$ e 
...
\begin{itemize}
    \item \textbf{Reconstrução dos grupos (linhas 15–18):} como o encoder 
    ...
    ordem (\texttt{\{linsin[b][1],linsin[b][0]\}}), reproduzindo em hardware 
    o deslocamento (\textit{shift}) necessário para realinhar a máscara $SCb$ 
    com a Região 3, conforme descrito no Critério de Seleção 
    (Tabela~\ref{tab:criterio_regiao}).
\end{itemize}
...
module fpga_top_design( 
    ...
    assign fpga_mem_io_up =
           (write_en==1'b0 && chip_sel_out[1]==1'b0) ? encoder_output_up : 16'bz;

    // Escrita na Memoria 2 (DOWN): Ativa o driver se write_en=1 e chip_sel_out[0]=0
    assign fpga_mem_io_down =
           (write_en==1'b0 && chip_sel_out[0]==1'b0) ? encoder_output_down : 16'bz;
...
O ciclo de leitura exige uma inversão completa no controle de posse dos barramentos. O microcontrolador altera o estado para \texttt{write\_en = 1} e recolhe os seus \textit{drivers}. Ao detectar esta mudança, o Top-Level desativa os \textit{buffers} conectados às memórias, permitindo que as SRAMs externas tomem posse dos fios físicos e conduzam os 32 bits armazenados para dentro da FPGA.
...
A Figura \ref{fig:diagrama_blocos} consolida visualmente a hierarquia de roteamento descrita nas subseções anteriores. O "Controle Externo" (o STM32) governa as operações injetando os quatro sinais de configuração -- \texttt{write\_en}, \texttt{chip\_sel}, \texttt{output\_en} e \texttt{ecc\_sel[2:0]} -- diretamente no "Top Multiplexador".
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[node distance=2cm, auto, >=Latex]
        ...
        \node [block, right=4.2cm of master] (topmux) {Top Multiplexador};
        ...
        \draw [->, thick] (master) -- node[above, scale=0.72] {\shortstack{\texttt{write\_en}, \texttt{chip\_sel},\\ \texttt{output\_en}, \texttt{ecc\_sel[2:0]}}} (topmux);
...
(topmux.north)
    ...
    -- ($(mem1.north)+(0,1.8)$)
    -- (mem1.north);
\node[anchor=south east, scale=0.8, red!70!black, xshift=-2pt, yshift=2pt]
    at ($(mem1.north)+(0,1.8)$) {Bypass 1 (16-bit)};
...
(topmux.south)
    ...
    -- ($(mem2.south)+(0,-1.8)$)
    -- (mem2.south);
\node[anchor=north east, scale=0.8, red!70!black, xshift=-2pt, yshift=-2pt]
    at ($(mem2.south)+(0,-1.8)$) {Bypass 2 (16-bit)};
    \end{tikzpicture}%
    }
...
\subsection{Exemplos de Fluxo de Dados por Modo de Operação}
\label{sec:exemplos_fluxo}

Para ilustrar na prática como o sinal \texttt{ecc\_sel} determina o caminho 
físico percorrido pelo dado dentro da FPGA, esta seção apresenta dois 
recortes do diagrama estrutural (Figura~\ref{fig:diagrama_blocos}), cada um 
mostrando \textbf{apenas os blocos e conexões efetivamente ativos} durante 
um ciclo de escrita ($\texttt{write\_en} = 0$) em um modo de operação 
específico. Os demais blocos (o outro algoritmo de ECC, a memória não 
selecionada, as vias de \textit{bypass} não utilizadas) são omitidos, já 
que nenhum dado trafega por eles nesse cenário.

\subsubsection{Exemplo: Escrita com o Algoritmo TBEC-RSC Selecionado}

Neste cenário, o firmware do STM32 configura \texttt{ecc\_sel = 3'b010}, 
acionando o modo de operação do Nível 2 (Seção~\ref{sec:nivel2_multiplexador}) 
que instancia o \texttt{TBEC\_RSC\_encoder}. O dado de 16 bits vindo do 
processador (\texttt{data\_in\_left}) é codificado em um \textit{codeword} 
de 32 bits, fatiado e escrito simultaneamente nas duas memórias -- por isso 
\texttt{chip\_sel\_out} habilita ambas ao mesmo tempo.
\begin{figure}[h!]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[node distance=2cm, auto, >=Latex]
        \tikzstyle{block} = [rectangle, draw, fill=blue!10, text width=2.6cm, text centered, rounded corners, minimum height=1.1cm, draw=brandblue, thick]
        
        \node [block] (master) {Controle Externo};
        % Gaps reduzidos para o diagrama nao estourar a pagina e a fonte ficar grande
        \node [block, right=3.2cm of master] (topmux) {Top Multiplexador};
        \node [block, right=2.2cm of topmux, fill=green!5] (ecc1) {ECC 1 (TBEC)};
        \node [block, above right=0.8cm and 3.5cm of ecc1, fill=orange!10] (mem1) {Memória 1 (Up)};
        \node [block, below right=0.8cm and 3.5cm of ecc1, fill=orange!10] (mem2) {Memória 2 (Down)};
        
        \draw [->, thick] (master) -- node[above, scale=0.9, align=center] {\texttt{ecc\_sel=3'b010}\\ \texttt{write\_en=0}\\ \texttt{chip\_sel=1}} (topmux);
        
        \draw [->, thick] (topmux) -- node[above, scale=0.9, align=center] {\texttt{data\_in\_left}\\ (16-bit)} (ecc1);
        
        % Ajuste: Texto ancorado exatamente na borda da memoria, crescendo para a ESQUERDA (left)
        \draw [->, thick, draw=blue!70!black] (ecc1.east) -- ++(1.0,0) |- (mem1.west);
        \node [above left, scale=0.9, text=blue!70!black] at (mem1.west) {\texttt{encoder\_output1[0:15]}};
        
        \draw [->, thick, draw=green!60!black] (ecc1.east) -- ++(1.0,0) |- (mem2.west);
        \node [below left, scale=0.9, text=green!60!black] at (mem2.west) {\texttt{encoder\_output1[16:31]}};
        
        \node[scale=0.9, below=0.35cm of topmux] {\texttt{chip\_sel\_out = \{1,1\}}};
    \end{tikzpicture}%
    }
    \caption{Caminho de dados ativo durante uma escrita com o algoritmo TBEC-RSC selecionado (\texttt{ecc\_sel = 3'b010}). O ECC 2 (MRSC), as vias de \textit{bypass} e o caminho de leitura (decoder) não participam desta operação e por isso não aparecem.}
    \label{fig:exemplo_tbec}
\end{figure}

\subsubsection{Exemplo: Escrita com o Modo Bypass Selecionado}

Neste segundo cenário, o firmware configura \texttt{ecc\_sel = 3'b000}, 
selecionando o modo \textit{bypass} para a Memória~1: o dado de 16 bits é 
encaminhado diretamente do processador para a SRAM, sem passar por nenhum 
codificador. Como apenas a Memória~1 é endereçada, \texttt{chip\_sel\_out} 
habilita somente essa memória, mantendo a Memória~2 em alta impedância.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[node distance=2cm, auto, >=Latex]
        ...
        % Setas fixadas em 5.0cm. Isso garante que:
        % 1. Fiquem exatamente do mesmo tamanho.
        % 2. A largura total do diagrama seja de 17.8cm (idêntica ao diagrama TBEC).
        % 3. O resizebox aplique a mesma escala, igualando perfeitamente o tamanho da fonte!
        \node [block, right=5.0cm of master] (topmux) {Top Multiplexador};
        \node [block, right=5.0cm of topmux, fill=orange!10] (mem1) {Memória 1 (Up)};
        
        \draw [->, thick] (master) -- node[above, scale=0.9, align=center] {\texttt{ecc\_sel=3'b000}\\ \texttt{write\_en=0}\\ \texttt{chip\_sel=1}} (topmux);
        
        \draw [->, thick, dashed, draw=red!70!black] (topmux) -- 
            node[above, scale=0.9, text=red!70!black, align=center] {\texttt{data\_in\_left} $\to$\\ \texttt{data\_out\_right\_up}\\ (16-bit, sem ECC)} (mem1);
        
        \node[scale=0.9, below=0.35cm of topmux] {\texttt{chip\_sel\_out = \{1,0\}}};
    \end{tikzpicture}%
    }
    \caption{Caminho de dados ativo durante uma escrita em modo \textit{bypass} direto para a Memória~1 (\texttt{ecc\_sel = 3'b000}). Nenhum dos dois algoritmos de ECC, nem a Memória 2, participam desta operação.}
    \label{fig:exemplo_bypass}
\end{figure}
...
O STM32 desempenha um papel que vai além da simples supervisão: ele é o 
\textbf{agente responsável por todo o fluxo de realização do experimento}. 
É o STM32 quem escreve os dados nas memórias (e mantém o registro do que 
foi escrito), quem realiza leituras periódicas para verificar a integridade 
dessa informação ao longo do tempo, e quem acompanha a incidência dos erros via o vetor \texttt{flag\_out} (vetor responsável por sinalizar em que região do MRSC o erro foi detectado), além de ser o único 
elemento com conhecimento de mais alto nível, é necessário para o estudo de taxa de erro (percebe-se que, mesmo que o vetor seja destinado a localização do erro, em termos de região, o código também sinaliza quando não incidiu em nenhuma região, o que possibilita o estudo da taxa de erro, logicamente).
...
tratamento de interrupção ou telemetria de região do erro.
