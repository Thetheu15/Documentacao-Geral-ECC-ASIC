...
\begin{itemize}
    ...
    \item \textbf{Bit a Bit / \textit{Bitwise} (\texttt{\&}, \texttt{|}, \texttt{\^}, \texttt{\~}):} Operam individualmente e paralelamente em cada bit de um vetor. O operador XOR (\texttt{\^}) é a base matemática fundamental para a construção de qualquer gerador de paridade ou código corretor de erros (ECC).
...
[Resumo: Detalhar as propriedades físicas do setup, identificando o modelo exato e os recursos de barramento da placa utilizada no projeto].

\subsection{Cenários de Validação e Requisitos de Teste}
[Resumo: Definir os parâmetros para atestar o funcionamento correto do circuito, cobrindo simulações lógicas e metodologias de injeção de falhas controladas].
...
[Resumo: Explicar o papel do firmware supervisor hospedado na CPU externa, responsável pelo gerenciamento de interrupções e reconfiguração dinâmica do módulo via GPIO/barramento].

\subsection{Interface com Memória Externa (SRAM)}
[Resumo: Apresentar a dinâmica de leitura e escrita direta nos blocos de memória estática obedecendo às restrições físicas de tempos de ciclo].

% -------------------------------------------------------------------------
\section{Resultados e Considerações Finais}
\label{sec:conclusao}
% -------------------------------------------------------------------------
[Resumo: Espaço reservado para resumir a validação do protótipo, métricas de ocupação de área na síntese e os próximos passos para otimização da arquitetura].
